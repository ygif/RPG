...
public class MegaOgre extends Ogre {
	...
	public MegaOgre(String name, int level, int health, int attack, int defense, int speed) {
		...
		atk = new NormAtk(1.5, "normal attack", "The " + name + " attacks the player.", this);
		club = new NormAtk(1.8, "club hit", "The " + name + " hits the player with a club.", "Use a club to hit a target", this);
		smash = new NormAtk(2.1, "ogre smash", "The " + name + " smashes the player.", "A strong punch to the target", this);
		ram = new RCAttack(2.5, "ram", "The " + name + " rams itself into the player", "Run into the target as fast as possible. Damages the user.", this);
	}
